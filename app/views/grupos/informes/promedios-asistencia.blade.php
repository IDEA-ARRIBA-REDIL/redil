...
<html>
  <head>
      <meta charset="UTF-8">
      <title>{{Lang::get('reporte_grupos.texto_reporte_index_titulo_pagina')}} | {{ Lang::get('grupos.informe_promedio_asistencia_titulo') }}</title>
        ...
        <div class="box box-filtro" >
          <div class="box-header">
            <h3 class="content-header barra-titulo">
              <span class="mayusculas">{{ Lang::get('grupos.informe_promedio_asistencia_titulo') }}</span><br>
              <small>{{ Lang::get('grupos.informe_promedio_asistencia_descripcion') }} </small>
            </h3>  
          </div>
                  ...
                  <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
                    ...
                    <form id="filtros" action="" method="get" role="form" class="form-inline">
                      ...
                      <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12 campo-filtro">
                        <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8 no-padding" style="padding-right: 5px !important;">
                          <select style="width:100%" id="ordenado-por" name="ordenado-por" class="form-control" data-placeholder="Filtro por línea" >
                            <option value="grupos.id" @if(isset($ordenado_por)) @if($ordenado_por=="grupos.id") selected @endif @endif >Ordenado por Cod. Asistente</option>
                            <option value="promedio" @if(isset($ordenado_por)) @if($ordenado_por=="promedio") selected @endif @endif >Ordenado por Promedio del Periodo (PP)</option>
                            <!--<option value="puntaje" @if(isset($ordenado_por)) @if($ordenado_por=="puntaje") selected @endif @endif >Ordenado por Promedio y Cantidad de Reportes del Periodo (PP y CR)</option>-->
                          </select>
                        </div>
                        <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 no-padding">
                          <select style="width:100%" id="orden" name="orden" class="form-control" data-placeholder="Filtro por línea" >
                            <option value="asc" @if(isset($orden)) @if($orden=="asc") selected @endif @endif >De menor a mayor</option>
                            <option value="desc" @if(isset($orden)) @if($orden=="desc") selected @endif @endif >De mayor a menor</option>
                          </select>
                        </div>
                      </div>
                          ...
                          <tr>                                                    
                              ...
                              @for($i=$mes_inicio; $i<=$mes_fin; $i++)
                                @if(strtotime($mes_requerido) <= strtotime(Helper::finalMes(date("Y"), date("m"))))
                                  <td style="border-left-width: 3px;vertical-align: middle; background:#f3f4f5;" align="center"> {{ (int)$grupo->promedioAsistencia(date('Y-m-d', strtotime($mes_requerido)), Helper::finalMes(date("Y",strtotime($mes_requerido)), date("m",strtotime($mes_requerido)))) }}</td>
                                @else
              ...
              <div class="box-footer">                                   
                <div class="row">                                        
                    <div class="col-lg-4">                                            
                      ...
                      <div class="col-lg-8 text-right" style="padding-right: 30px!important;"> {{ $grupos->appends(array('buscar' => $buscar, 'rango' => $rango, 'orden' => $orden, 'ordenado-por' => $ordenado_por, 'anio' => $anio, 'linea' => $linea))->links() }}</div>
