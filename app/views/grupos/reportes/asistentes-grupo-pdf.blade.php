...
<body>
    ...
    <div id="header">
        <B id="titulo">{{Lang::get('grupos.texto_titulo_pagina_pdf')}}</B> <br> <b>{{Lang::get('grupos.lg_lb_codigo')}}.</b> {{$grupo->id}} / <b>{{Lang::get('grupos.texto_simple_grupo')}}:</b> {{$grupo->nombre }} / 
        @if (isset($grupo->linea()->nombre))
        <b>{{Lang::get('grupos.texto_simple_termino_grupo_plural')}}: </b> {{$grupo->linea()->nombre}} 
        @endif
    </div>
    ...
    <div id="content">
        <table style='border-top: 1px solid #dddddd;' cellspacing=0 cellpadding=4 bordercolor='#aaa' width='100%' >
            <thead>
                <tr>
                    <th><i class='fa fa-warning'> </i><span class="mayusuculas">CÓD.</span></th>
                    <th>NOMBRE</th>  
                    <th>TELEFONO (S)</th>
                    <th>DIRRECCIÓN </th> 
                    <th >OBSERVACIÓN</th>
            ...
            @foreach($grupo->asistentes as $asistente)
                <tr style='border-top: 1px solid #dddddd;'>
                    ...
                    <td  id="titulo" style='border-top: 1px solid #dddddd;'>
